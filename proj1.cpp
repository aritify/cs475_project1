...
main( int argc, char *argv[ ] )
{
	...
	for( int tries = 0; tries < NUMTRIES; tries++ )
	{
		...
		for( int n = 0; n < NUMTRIALS; n++ )
		{
			...
			float t = -vy / (0.5 * GRAVITY);	// time ball is at ground level
			float x = vx * t;					// distance ball went horizontally at time t
			if( x <= g )						// if x is less than g(cliff face)
			{
			...
			else
			{
				...
				t = g / vx;								// time ball is even with g(cliff face)
				float y = vy*t + 0.5*GRAVITY*(t*t);		// how vertical the ball went at time t
				if( y <= h )							// if y is less than h(cliff height)
				{
				...
				else
				{
					...
					//       'B' multiplies time
					//       'C' is a constant
					// 0.5*Gravity*(t*t) + vy*t - h = 0; alternate form of y
					float A = 0.5*GRAVITY;
					float B = vy;
					float C = -h;
